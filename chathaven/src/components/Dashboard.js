...
useEffect(() => {
    if (selectedChannel) {
        fetch(`/api/channels/users?channelId=${selectedChannel._id}`)
            .then((res) => res.json())
            .then((data) => setChannelUsers(data))
            .catch((error) => console.error("Error fetching channel users:", error));
    }
}, [selectedChannel]);
...
useEffect(() => {
  ...
  return (
    <div id="dashboardContainer">
      ...
      {user?.isGlobalAdmin && selectedChannel && (
  <div className="admin-user-dropdown">
    <label className="dropdown-label">Users in {selectedChannel.name}:</label>

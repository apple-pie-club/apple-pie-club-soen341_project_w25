...
export default function DashboardPage() {
  ...
  return (
    <div id="dashboardContainer">
      ...
      {/* Chat Window */}
      {selectedChannel && <CMsWindow selectedChannel={selectedChannel} messageAreaClass={getMessageAreaClass()} user={user} />}
      ...
      {isMenuOpen && (
        <CreateTeamMenu isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} onCreateTeam={(newTeam) => setTeams((prevTeams) => [...prevTeams, newTeam])} />
      )}
      {isCreateChannelModalOpen && (
        <CreateChannelMenu isOpen={isCreateChannelModalOpen} onClose={() => setIsCreateChannelModalOpen(false)} teamMembers={selectedTeam ? selectedTeam?.members : []} onCreateChannel={handleCreateChannel} teamId={selectedTeam ? selectedTeam._id : ""} />
      )}
    </div>
  );
}
